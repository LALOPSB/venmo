...
RSpec.describe User do
  ...
  describe 'activity feed' do
    ...
    let!(:pay_to_friend) { FactoryBot.create(:feed_item, sender: user, friend: friend_1, amount: 80, description: 'Payment to friend') }
    let!(:pay_between_friends) { FactoryBot.create(:feed_item, sender: friend_1, friend: friend_2, amount: 15, description: 'Payment between friends') }
    ...
    let!(:pay_from_friend) { FactoryBot.create(:feed_item, sender: friend_1, friend: user, amount: 108, description: 'Payment from friend') }

    context 'build activity feed' do
      let(:expected_non_friend_2_activity_feed) do
        [
          "#{non_friend_1.username} paid #{non_friend_2.username} on #{pay_between_non_friends.created_at} - $23 - Payment between non friends",
